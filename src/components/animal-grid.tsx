...
const animalDefs: readonly AnimalDef[] = [
  { key: "bee",      audio: "bee",      image: "/bee.png" },
  { key: "bird",     audio: "bird",     image: "/bird.jpg" },
  { key: "cat",      audio: "cat",      image: "/cat.jpg" },
  { key: "chicken",  audio: "chicken",  image: "/chicken.jpg" },
  { key: "cow",      audio: "cow",      image: "/cow.jpg" },
  { key: "dog",      audio: "dog",      image: "/dog.jpg" },
  { key: "elephant", audio: "elephant", image: "/elefant.png" },
  { key: "fish",     audio: "fish",     image: "/fish.png" },
  { key: "horse",    audio: "horse",    image: "/hourse.jpg" },
  { key: "lion",     audio: "lion",     image: "/lion.jpg" },
  { key: "pig",      audio: "pig",      image: "/pig.png" },
  { key: "turtle",   audio: "turtle",   image: "/turtule.jpg" },
] as const;

const animalKeys = animalDefs.map((a) => a.key);

interface AnimalGameTexts {
...
export function AnimalGrid({ texts }: { texts: AnimalGameTexts }) {
  const animals = useMemo(() => shuffle([...animalDefs]), []);
  const [gameState, setGameState] = useState<GameState>("idle");
  const [targetAnimal, setTargetAnimal] = useState<string | null>(null);
  ...
  const availableKeys = animals.filter((a) => !removed.has(a.key));

  const startRound = useCallback(() => {
    const available = animalKeys.filter((k) => !removed.has(k));
    if (available.length === 0) return;
    const animal = pickRandom(available);
    setTargetAnimal(animal);
    setGameState("playing");
    const audioDef = animalDefs.find((a) => a.key === animal);
    if (audioDef) setTimeout(() => playAudio(audioDef.audio), 1000);
  }, [playAudio, removed]);
  ...
  const handleAnimalClick = useCallback(
    (key: string, audio: string) => {
      ...
      if (key === targetAnimal) {
        ...
        const remaining = animalKeys.filter((k) => !newRemoved.has(k));
        if (remaining.length === 0) {
          ...
          setTimeout(() => {
            ...
            setGameState("playing");
            const audioDef = animalDefs.find((a) => a.key === next);
            if (audioDef) setTimeout(() => playAudio(audioDef.audio), 1000);
          }, 1200);
        }
      } else {
    ...
    [gameState, targetAnimal, playAudio, removeMode, removed]
  );
  ...
  return (
    <>
        ...
        {gameState !== "idle" && (
          <>
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg sm:text-xl font-extrabold text-slate-800 animate-[fadeSlideUp_0.3s_ease-out_both]" key={targetAnimal}>
                {targetAnimal ? texts.labels[targetAnimal] : ""}
              </span>
            </div>
            {gameState === "correct" && (
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 grid-rows-4 md:grid-rows-2 gap-3 sm:gap-4 flex-1 min-h-0 max-h-[75dvh] self-center w-full">
        {animals.map(({ key, audio, image }, index) => {
          const isPlaying = gameState === "playing";
          const isRemoved = removed.has(key);

          return (
            <div key={key} onClick={() => !isRemoved && handleAnimalClick(key, audio)}
              ...
              <div className="absolute inset-2 sm:inset-3 rounded-xl overflow-hidden bg-white flex items-center justify-center transition-transform duration-700 ease-out group-hover:scale-105">
                <Image src={image} alt={texts.labels[key]} width={300} height={300} className="w-full h-full object-contain p-1" />
              </div>
            </div>
          );
        })}
